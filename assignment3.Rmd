...
df <- data.frame(parameters, seed1.5$pooled[,10], seed1.10$pooled[,10],
                 ...
                 seed2.5$pooled[,10], seed2.25$pooled[,10])
...
We have $411$ observations with observed values on all $12$ variables. Also, $10$ observations for which on `WC` is missing, $6$ observations for which on `hgt` is missing Interestingly, we can observe that there are $29$ observations for which on `chol`, `HDL` and `bili` are missing.

As a matter of example, in the previous plot depicting the distribution of the observed data for the different variables, we could appreciate that using a normal distribution for the `hgt` is possibly not a completely unreasonable idea. Let us then change the default imputation method from `pmm` to `norm` for the variable `hgt`.
...
<!-- In addition, we further scrutinise in the domain of biomedical, `chol` is the sum of `HDL` and `LDL`. Therefore we can conclude that the missingness depends on the value `HDL`. This is also evident where the number of missing values in `chol` and `HDL` is identical and they have similar looking density above. -->