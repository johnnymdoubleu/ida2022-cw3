...
pool1 <- pool(with(mice(nhanes, printFlag = F, seed = 1), 
                   lm(bmi ~ as.factor(age) + as.factor(hyp) + chl)))
...
Now to perform the above statistics, we first took `age` and `hyp` as categorical variables and assign them as factor. From Table 1, we computed the pooled estimates, degree of freedom and lambda values. This `lambda` represents the proportions of variance due to the missing data for each parameter. We can observe that`age` factors generally have higher $\lambda$ values than the others. Especially, `age(3)` had the highest value of $0.6819661$. We need to note that the intercept contains `age(1)` and `hyp(1)` but having the lowest $\lambda$ value of $0.2318567$. Hence, we conclude that `age` is mostly affected by the nonresponse.
...
Here we repeated the same step as Q1 b) with different seed. From Table 2, we can see that the most affected variables will change with different number of settings. For instance, in `seed=5`, `hyp(2)` has the highest $\lambda$ value of $0.5858550$ whereas `seed=2,3 and 6` have `age` factors as the most affected variable. This is unstable throughout different seed because there percentage of imputation we found in Q1 a) is considerably high with $48\%$.
